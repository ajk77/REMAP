...
NAVIGATION: 
	TABLE BUILD ORDER 
	REMAP_CA.v3ModifiedENCOUNTER_PHI -> FROM CT_DATA.ENCOUNTER_PHI, REMAP_CA.ManualChange_Encounter_FIN
	REMAP_CA.v3Participant -> FROM REMAP_CA.v3ViewCernerEnrolledPerson2
	REMAP_CA.v3IdMap -> FROM REMAP_CA.v3ViewCernerEnrolledPerson2, CT_DATA.ENCOUNTER_ALL, CT_DATA.CODE_VALUE  
	REMAP_CA.v3LocOrder -> FROM CT_DATA.ENCNTR_LOC_HIST, REMAP_CA.v3IdMap
	REMAP_CA.v3tempLastEvent -> FROM for determing undocumented discharge >> used for updating REMAP_CA.v3LocOrder to close off open loc entries
	REMAP_CA.v3Lab -> FROM CT_DATA.CE_LAB, REMAP_CA.v3IdMap, COVID_SUPPLEMENT.CV_STANDARDIZATION, CT_DATA.CODE_VALUE
	REMAP_CA.tempv3Physio -> FROM CT_DATA.CE_PHYSIO
	REMAP_CA.v3Physio -> FROM REMAP_CA.tempv3Physio, REMAP_CA.v3IdMap, COVID_SUPPLEMENT.CV_STANDARDIZATION, CT_DATA.CODE_VALUE
	REMAP_CA.v3PhysioStr -> FROM REMAP_CA.tempv3Physio, REMAP_CA.v3IdMap, COVID_SUPPLEMENT.CV_STANDARDIZATION, CT_DATA.CODE_VALUE
	REMAP_CA.v3IO -> FROM CT_DATA.CE_INTAKE_OUTPUT_RESULT, REMAP_CA.v3IdMap, COVID_SUPPLEMENT.CV_STANDARDIZATION
	REMAP_CA.v3Med -> FROM CT_DATA.MAR_AD, REMAP_CA.v3IdMap, COVID_SUPPLEMENT.CV_STANDARDIZATION, CT_DATA.CODE_VALUE
	REMAP_CA.v3OrganSupportInstance -> FROM REMAP_CA.v3Med, REMAP_CA.v3PhysioStr, REMAP_CA.v3Physio, REMAP_CA.NIVexclusion
	REMAP_CA.v3RandomizedSevere -> FROM REMAP_CA.v3Participant, REMAP_CA.v3LocOrder, REMAP_CA.v3OrganSupportInstance, COVID_SUPPLEMENT.UNIT_DESCRIPTION_ARCHIVE (updated from REMAP_CA.HistoricRandomizationTimes)
	REMAP_CA.v3RandomizedModerate -> FROM REMAP_CA.v3Participant, REMAP_CA.v3RandomizedSevere (updated from REMAP_CA.HistoricRandomizationTimes)
	REMAP_CA.v3StudyDay -> FROM REMAP_CA.v3RandomizedModerate, REMAP_CA.v3RandomizedSevere, COVID_SUPPLEMENT.STUDY_DAY, REMAP_CA.v3LocOrder
	REMAP_CA.v3tempDefinedLocOrder -> FROM REMAP_CA.v3LocOrder, REMAP_CA.v3OrganSupportInstance, COVID_SUPPLEMENT.UNIT_DESCRIPTION_ARCHIVE, CT_DATA.CODE_VALUE
	REMAP_CA.v3UnitStay -> FROM REMAP_CA.v3tempDefinedLocOrder
	REMAP_CA.v3IcuStay -> FROM REMAP_CA.v3UnitStay
	REMAP_CA.v3RRTInstance -> FROM REMAP_CA.v3IO, REMAP_CA.v3Physio
	REMAP_CA.v3SupplementalOxygenInstance -> FROM REMAP_CA.v3PhysioStr , REMAP_CA.v3Physio
	REMAP_CA.v3CalculatedHourlyFiO2 -> FROM REMAP_CA.tempPnearestF, REMAP_CA.tempPnearestE
	REMAP_CA.tempPnearestF -> FROM REMAP_CA.v3Lab, REMAP_CA.v3CalculatedHourlyFiO2  
	REMAP_CA.tempPnearestE -> FROM REMAP_CA.v3Lab, REMAP_CA.v3Physio
	REMAP_CA.v3CalculatedPFratio -> FROM REMAP_CA.tempPnearestF, REMAP_CA.tempPnearestE
	REMAP_CA.v3CalculatedPEEPjoinFiO2 -> FROM REMAP_CA.v3Physio, REMAP_CA.v3CalculatedHourlyFiO2 
	REMAP_CA.v3CalculatedStateHypoxiaAtEnroll -> FROM REMAP_CA.v3CalculatedPFratio, REMAP_CA.v3RandomizedModerate, REMAP_CA.v3RandomizedSevere, REMAP_CA.v3OrganSupportInstance
	REMAP_CA.v3CalculatedSOFA -> FROM REMAP_CA.v3Physio, REMAP_CA.v3StudyDay, REMAP_CA.v3Med, CT_DATA.MAR_AD MAR, CT_DATA.CODE_VALUE, REMAP_CA.v3Physio
	REMAP_CA.v3Hospitalization -> REMAP_CA.v3Participant, REMAP_CA.v3LocOrder, CT_DATA.ENCOUNTER_ALL, REMAP_CA.v3IdMap, REMAP_CA.ManualChange_StartOfHospitalization_utc, COVID_PHI.Locked_HospDisposition, COVID_PHI.Vital_Status_LTOC_to_COVID_Server
	REMAP_CA.v3IcuAdmitDaysOnSupport -> FROM REMAP_CA.v3IcuStay, REMAP_CA.v3UnitStay, REMAP_CA.v3OrganSupportInstance, REMAP_CA.v3RandomizedModerate, REMAP_CA.v3RandomizedSevere
	REMAP_CA.v3IcuDaysOnSupport -> FROM REMAP_CA.v3IcuAdmitDaysOnSupport

*/

/*
Dependencies
>REMAP_CA.v3ViewCernerEnrolledPerson2 from REMAP_CA-v3-definedViews.sql
>to_utc(), to_float(), get_prefix(), get_postfix(), get_physio_result_str() from REMAP_CA.v3-definedFunctions.sql
*/
...
CREATE TABLE REMAP_CA.v3ModifiedENCOUNTER_PHI  
	...
	FROM CA_DATA.ENCOUNTER_PHI EP
	LEFT JOIN REMAP.ManualChange_Encounter_FIN MC ON EP.ENCNTR_ID = MC.ENCNTR_ID  /* OLD REMAP line */ 
...
DROP TABLE REMAP_CA.v3CernerEnrolledPerson2; 
CREATE TABLE REMAP_CA.tempENCOUNTERS
	SELECT 	   
	 	EA.PERSON_ID, 
		EA.ENCNTR_ID,
	...
	FROM  
	   (
			...
			JOIN REMAP_CA.v3ModifiedENCOUNTER_PHI EP ON EF.FIN = LPAD(EP.fin, 13, '0')
			JOIN CA_DATA.ENCOUNTER_ALL EA ON EP.encntr_id = EA.encntr_id
		) AS all_screendates
		LEFT JOIN CA_DATA.ENCOUNTER_ALL as EA  ON EA.PERSON_ID = all_screendates.PERSON_ID
; 
CREATE TABLE REMAP_CA.v3CernerEnrolledPerson2 
	SELECT
	   TE.PERSON_ID,
	   #E.MRN,
	   E.ENCNTR_ID,
	   ...
	   TE.REGIMEN
	FROM
		REMAP_CA.tempENCOUNTERS AS TE
	   LEFT JOIN REMAP_CA.v3ModifiedENCOUNTER_PHI as E on E.ENCNTR_ID = TE.ENCNTR_ID
	WHERE E.FIN IS NOT NULL 
;
DROP TABLE REMAP_CA.tempENCOUNTERS;

### pull studypateintids from view ###
DROP TABLE REMAP_CA.v3Participant;
CREATE TABLE REMAP_CA.v3Participant
	SELECT DISTINCT studypatientid, screendate_utc, regimen, '00000' as MRN, PERSON_ID, 'Cerner' AS source_system
	FROM REMAP_CA.v3CernerEnrolledPerson2
	ORDER BY studypatientid;

### link in related inpatient encntr_ids and fins. ###
DROP TABLE REMAP_CA.v3IdMap;	
CREATE TABLE REMAP_CA.v3IdMap
...
DROP TABLE REMAP_CA.v3LocOrder;
CREATE TABLE REMAP_CA.v3LocOrder
	...
	FROM
		(SELECT DISTINCT STUDYPATIENTID, ELH.encntr_id,
			REMAP.to_utc( ELH.beg_effective_dt_tm) AS beg_utc,   /* OLD REMAP line */ 
			REMAP.to_utc( ELH.end_effective_dt_tm) AS end_utc,   /* OLD REMAP line */ 
			ELH.LOC_FACILITY_CD, ELH.LOC_NURSE_UNIT_CD
		FROM CA_DATA.ENCNTR_LOC_HIST ELH 
		JOIN REMAP_CA.v3IdMap M ON ELH.ENCNTR_ID = M.ENCNTR_ID
		) AS IR
	;  
	
	## close locations without an end time ##
	CREATE TABLE REMAP_CA.v3tempLastEvent
		WITH encntr_id_list AS
		(SELECT DISTINCT encntr_id FROM REMAP_CA.v3IdMap
		)
		SELECT encntr_id, REMAP.to_utc(MAX(max_event_dt)) AS max_event_utc  /* OLD REMAP line */
			FROM
				(
					SELECT 'lab', encntr_id, MAX(event_end_dt_tm) AS max_event_dt  FROM CA_DATA.CE_LAB 
					WHERE encntr_id IN (SELECT encntr_id FROM REMAP_CA.v3IdMap) GROUP by encntr_id
				UNION
					SELECT 'physio', encntr_id, MAX(event_end_dt_tm) AS max_event_dt FROM CA_DATA.CE_PHYSIO 
					WHERE encntr_id IN (SELECT encntr_id FROM REMAP_CA.v3IdMap) GROUP by encntr_id
				UNION
					SELECT 'med', encntr_id, MAX(event_end_dt_tm) AS max_event_dt FROM CA_DATA.CE_MED 
					WHERE encntr_id IN (SELECT encntr_id FROM REMAP_CA.v3IdMap) GROUP by encntr_id
				) AS by_typedf
			GROUP BY encntr_id
		;
		# clost open loc entries up until most recent entry
		UPDATE REMAP_CA.v3LocOrder O1
			JOIN REMAP_CA.v3LocOrder O2 ON O1.studypatientid = O2.studypatientid AND O1.loc_order = O2.loc_order-1
			JOIN REMAP_CA.v3tempLastEvent LE ON O1.encntr_id = LE.encntr_id
		...
		;
	DROP TABLE REMAP_CA.v3tempLastEvent;

	## indicate which location entriy the pt was at time of screening ##
	UPDATE REMAP_CA.v3LocOrder O
		JOIN REMAP_CA.v3Participant P ON O.STUDYPATIENTID = P.STUDYPATIENTID
	...
	WITH max_end AS (
		...
		FROM REMAP_CA.v3LocOrder O1 
		LEFT JOIN REMAP_CA.v3LocOrder O2	ON O1.studypatientid = O2.studypatientid
...
DELETE FROM REMAP_CA.v3IdMap
	WHERE ENCNTR_ID NOT IN (SELECT DISTINCT ENCNTR_ID FROM REMAP_CA.v3LocOrder)
;

### pull relevant labs ###
DROP TABLE REMAP_CA.v3Lab;
CREATE TABLE REMAP_CA.v3Lab
	SELECT DISTINCT L.EVENT_ID, M.STUDYPATIENTID, L.event_utc, S.sub_standard_meaning, L.prefix, L.result_float, 
		CV.display as units, L.RESULT_VAL AS documented_val, NORMAL_LOW, NORMAL_HIGH
	FROM  
		(SELECT DISTINCT EVENT_ID, ENCNTR_ID, EVENT_CD, /*REMAP.to_utc(*/CONVERT(EVENT_END_DT_TM, DATETIME)/*)*/ AS event_utc, 
			REMAP.get_prefix(RESULT_VAL) as prefix, REMAP.to_float(RESULT_VAL) AS result_float, result_units_cd,  /* OLD REMAP line */
			RESULT_VAL, NORMAL_LOW, NORMAL_HIGH
		FROM CA_DATA.CE_LAB
		WHERE encntr_id IN (SELECT encntr_id FROM REMAP_CA.v3IdMap)
			AND EVENT_CD IN (SELECT source_cv FROM COVID_SUPPLEMENT.CV_STANDARDIZATION WHERE source_table IN ('CE_LAB')) 
			AND EVENT_END_DT_TM != 0
		) AS L 
		JOIN REMAP_CA.v3IdMap M ON L.ENCNTR_ID = M.ENCNTR_ID
		JOIN COVID_SUPPLEMENT.CV_STANDARDIZATION S ON L.EVENT_CD = S.source_cv
		JOIN CT_DATA.CODE_VALUE CV ON L.result_units_cd = CV.code_value  /* OLD REMAP line */
	WHERE result_float IS NOT NULL
;  

### pull relevant physio ###
CREATE TABLE REMAP_CA.tempv3Physio
	SELECT DISTINCT EVENT_ID, ENCNTR_ID, EVENT_CD, CONVERT(EVENT_END_DT_TM, DATETIME) AS EVENT_END_DT_TM, RESULT_VAL, result_units_cd, NORMAL_LOW, NORMAL_HIGH 
		FROM CA_DATA.CE_PHYSIO
		WHERE encntr_id IN (SELECT encntr_id FROM REMAP_CA.v3IdMap)
	...
	DROP TABLE REMAP_CA.v3Physio;
	CREATE TABLE REMAP_CA.v3Physio
		SELECT DISTINCT L.EVENT_ID, M.STUDYPATIENTID, L.event_utc, S.sub_standard_meaning, L.prefix, L.result_float, 
			CV.display AS units, L.RESULT_VAL AS documented_val, NORMAL_LOW, NORMAL_HIGH
		FROM  
			(SELECT EVENT_ID, ENCNTR_ID, EVENT_CD, EVENT_END_DT_TM AS event_utc, 
				REMAP.get_prefix(RESULT_VAL) as prefix, REMAP.to_float(RESULT_VAL) AS result_float, result_units_cd,  /* OLD REMAP line */
				RESULT_VAL, NORMAL_LOW, NORMAL_HIGH
			FROM REMAP_CA.tempv3Physio
			) AS L 
			JOIN REMAP_CA.v3IdMap M ON L.ENCNTR_ID = M.ENCNTR_ID
			JOIN COVID_SUPPLEMENT.CV_STANDARDIZATION S ON L.EVENT_CD = S.source_cv
			JOIN CT_DATA.CODE_VALUE CV ON L.result_units_cd = CV.code_value  /* OLD REMAP line */
		WHERE result_float IS NOT NULL;
	DROP TABLE REMAP_CA.v3PhysioStr;
	CREATE TABLE REMAP_CA.v3PhysioStr
		SELECT DISTINCT L.EVENT_ID, M.STUDYPATIENTID, L.event_utc, S.sub_standard_meaning, 
			REMAP.get_physio_result_str(S.sub_standard_meaning, L.result_val) AS result_str, CV.display AS units, L.result_val AS documented_text  /* OLD REMAP line */
		FROM  
			(SELECT EVENT_ID, ENCNTR_ID, EVENT_CD, EVENT_END_DT_TM AS event_utc, 
...
DROP TABLE REMAP_CA.tempv3Physio;

### pull relevant IO ###
DROP TABLE REMAP_CA.v3IO;
CREATE TABLE REMAP_CA.v3IO
	SELECT DISTINCT L.EVENT_ID, M.STUDYPATIENTID, L.event_utc, S.sub_standard_meaning, L.result_float, '' AS units
	FROM  
		(SELECT DISTINCT EVENT_ID, ENCNTR_ID, REFERENCE_EVENT_CD AS EVENT_CD, /*REMAP_CA.to_utc(*/CONVERT(IO_END_DT_TM, DATETIME) AS event_utc, 
			IO_VOLUME AS result_float
		FROM CA_DATA.CE_INTAKE_OUTPUT_RESULT
		WHERE encntr_id IN (SELECT encntr_id FROM REMAP_CA.v3IdMap)
...
### pull relevant meds ###
DROP TABLE REMAP_CA.v3Med;
CREATE TABLE REMAP_CA.v3Med
	SELECT DISTINCT L.EVENT_ID, M.STUDYPATIENTID, L.event_utc, S.sub_standard_meaning, L.ADMIN_DOSAGE, CVunit.display AS units, CVroute.display AS route
	FROM  
		(SELECT DISTINCT EVENT_ID, ENCNTR_ID, EVENT_CD, /*REMAP_CA.to_utc(*/CONVERT(ADMIN_START_DT_TM, DATETIME) AS event_utc, 
		...
		JOIN REMAP_CA.v3IdMap M ON L.ENCNTR_ID = M.ENCNTR_ID
		JOIN COVID_SUPPLEMENT.CV_STANDARDIZATION S ON L.EVENT_CD = S.source_cv
		JOIN CT_DATA.CODE_VALUE CVunit ON L.DOSAGE_UNIT_CD = CVunit.code_value  /* OLD REMAP line */
		JOIN CT_DATA.CODE_VALUE CVroute ON L.ADMIN_ROUTE_CD = CVroute.code_value  /* OLD REMAP line */
...
DROP TABLE REMAP_CA.v3OrganSupportInstance;
	CREATE TABLE REMAP_CA.v3OrganSupportInstance
				# VASO #
	   ...
	   UNION  # HFNC #
		...
		UNION # NIV #
			...
			LEFT JOIN REMAP.NIVexclusion E ON NIV.StudyPatientID = E.StudyPatientID  /* OLD REMAP line */
...
DROP TABLE REMAP_CA.v3RandomizedSevere;
CREATE TABLE REMAP_CA.v3RandomizedSevere
	WITH screening_loc AS (
		SELECT STUDYPATIENTID, loc_order FROM REMAP_CA.v3LocOrder WHERE screening_location = 1
	)
	...
	JOIN (
		SELECT DISTINCT L.studypatientid, GREATEST(MIN(O.event_utc), MIN(L.beg_utc)) AS earliest_ICU_organ_support_utc
		FROM REMAP_CA.v3LocOrder L
		JOIN screening_loc S ON L.studypatientid = S.studypatientid
		JOIN (SELECT * 
			FROM REMAP_CA.v3OrganSupportInstance 
...
DROP TABLE REMAP_CA.v3RandomizedModerate;
CREATE TABLE REMAP_CA.v3RandomizedModerate
	SELECT DISTINCT P.STUDYPATIENTID, P.screendate_utc as randomized_utc
	FROM REMAP_CA.v3Participant P
	LEFT JOIN REMAP_CA.v3RandomizedSevere R ON P.studypatientid = R.studypatientid
...
#DELETE FROM REMAP_CA.v3RandomizedModerate WHERE STUDYPATIENTID < 0400100212; updated on 1/15/21
DELETE FROM REMAP_CA.v3RandomizedModerate WHERE STUDYPATIENTID IN (
	SELECT DISTINCT STUDYPATIENTID FROM REMAP.HistoricRandomizationTimes);  /* OLD REMAP line */
INSERT INTO REMAP_CA.v3RandomizedModerate 
...
ALTER TABLE REMAP_CA.v3RandomizedModerate ORDER BY STUDYPATIENTID;

#DELETE FROM REMAP_CA.v3RandomizedSevere WHERE STUDYPATIENTID < 0400100212;
DELETE FROM REMAP_CA.v3RandomizedSevere WHERE STUDYPATIENTID IN (
	SELECT DISTINCT STUDYPATIENTID FROM REMAP.HistoricRandomizationTimes);  /* OLD REMAP line */
INSERT INTO REMAP_CA.v3RandomizedSevere 
	SELECT STUDYPATIENTID, RandomizedSevere_utc AS randomized_utc
	FROM REMAP.HistoricRandomizationTimes  /* OLD REMAP line */
	WHERE RandomizedSevere_utc IS NOT NULL;
ALTER TABLE REMAP_CA.v3RandomizedSevere ORDER BY STUDYPATIENTID;

### define study days within the first 30 ###
DROP TABLE REMAP_CA.v3StudyDay;
	CREATE TABLE REMAP_CA.v3StudyDay
		SELECT 
		...
		FROM 
			(SELECT 
				...
				CASE  
					...
					WHEN P.STUDY_DAY < 1 THEN REMAP.to_datetime_utc(DATE_ADD(P.randomized_date_local, INTERVAL P.STUDY_DAY DAY))   /* OLD REMAP line */
					ELSE REMAP.to_datetime_utc(DATE_ADD(P.randomized_date_local, INTERVAL P.STUDY_DAY-1 DAY))   /* OLD REMAP line */
				END AS day_start_utc,
				CASE
					WHEN P.STUDY_DAY = 0 THEN P.randomized_utc
					WHEN P.STUDY_DAY < 0 THEN REMAP.to_datetime_utc(DATE_ADD(P.randomized_date_local, INTERVAL P.STUDY_DAY+1 DAY))   /* OLD REMAP line */
					ELSE REMAP.to_datetime_utc(DATE_ADD(P.randomized_date_local, INTERVAL P.STUDY_DAY DAY))   /* OLD REMAP line */
				END AS day_end_utc,
				L.EndOfHospitalization_utc  
			FROM 
				( SELECT studypatientID, randomized_utc, DATE(REMAP.to_local(randomized_utc)) as randomized_date_local,    /* OLD REMAP line */
				...
				JOIN (SELECT studypatientid, if(MAX(end_utc) = '2100-12-31 00:00:00', NULL, MAX(end_utc)) AS EndOfHospitalization_utc
				 FROM REMAP_CA.v3LocOrder GROUP BY STUDYPATIENTID) AS L ON P.studypatientid = L.studypatientid
			) AS temp_result
		WHERE
...
### REMAP_CA.v3UnitStay ###
DROP TABLE REMAP_CA.v3UnitStay;
	CREATE TABLE REMAP_CA.v3tempDefinedLocOrder 
		...
		FROM REMAP_CA.v3LocOrder L1
		LEFT JOIN (
			SELECT DISTINCT L.studypatientid, L.loc_order, 1 AS includes_organSupport 
			FROM REMAP_CA.v3LocOrder L
			JOIN REMAP_CA.v3OrganSupportInstance O ON L.studypatientid = O.studypatientid
	...
	CREATE TABLE REMAP_CA.v3UnitStay
		WITH unitTransition AS  
			(SELECT D1.studypatientid, D1.beg_utc, D1.end_utc, D1.loc_order, D1.unit_type, D2.unit_type AS next_unit FROM
				REMAP_CA.v3tempDefinedLocOrder D1 JOIN REMAP_CA.v3tempDefinedLocOrder D2 
			...
			(SELECT studypatientid, MIN(loc_order) AS min_loc_order FROM REMAP_CA.v3tempDefinedLocOrder GROUP BY studypatientid
			), firstUnit AS # towards first
			(SELECT D.studypatientid, D.loc_order FROM REMAP_CA.v3tempDefinedLocOrder D JOIN minLocOrder m ON D.studypatientid = m.studypatientid
			...
			(SELECT studypatientid, MAX(loc_order) AS max_loc_order FROM REMAP_CA.v3tempDefinedLocOrder GROUP BY studypatientid
			), lastUnit AS # towards last
			(SELECT D.studypatientid, D.loc_order FROM REMAP_CA.v3tempDefinedLocOrder D JOIN maxLocOrder m ON D.studypatientid = m.studypatientid
			...
			(SELECT studypatientid, loc_order, includes_organSupport FROM REMAP_CA.v3tempDefinedLocOrder WHERE includes_organSupport = 1
			)
			...
			JOIN REMAP_CA.v3tempDefinedLocOrder Ds ON E.studypatientid = Ds.studypatientid AND E.loc_order_unit_start = Ds.loc_order
			JOIN REMAP_CA.v3tempDefinedLocOrder De ON E.studypatientid = De.studypatientid AND E.loc_order_unit_end = De.loc_order
	...
	UPDATE REMAP_CA.v3UnitStay U
	JOIN (SELECT * from REMAP_CA.v3tempDefinedLocOrder WHERE includes_stepdownUnit = 1
	...
	UPDATE REMAP_CA.v3UnitStay U
	JOIN (SELECT * from REMAP_CA.v3tempDefinedLocOrder WHERE includes_pandemicICU = 1
	) AS L ON U.studypatientid = L.studypatientid AND L.loc_order BETWEEN U.loc_order_unit_start AND U.loc_order_unit_end
	SET U.includes_pandemicICU = L.includes_pandemicICU;
	## drop temp table ##
	DROP TABLE REMAP_CA.v3tempDefinedLocOrder;

### identify ICU stays ###
DROP TABLE REMAP_CA.v3IcuStay; 
CREATE TABLE REMAP_CA.v3IcuStay
WITH organ_support_stay AS ( # identify ICU stays (including Stepdown/Pandemic ICU/ED stays with organ support)
	SELECT * 
	FROM REMAP_CA.v3UnitStay
	...
	LEFT JOIN REMAP_CA.v3OrganSupportInstance O ON S.StudyPatientid = O.StudyPatientid 
	...
	FROM core_rows C
	JOIN REMAP_CA.v3UnitStay U ON C.StudyPatientID = U.STUDYPATIENTID 
...
### v3RRTInstance ####
DROP TABLE REMAP_CA.v3RRTInstance;
	CREATE TABLE REMAP_CA.v3RRTInstance
...
DROP TABLE REMAP_CA.v3SupplementalOxygenInstance;
	CREATE TABLE REMAP_CA.v3SupplementalOxygenInstance
		SELECT device.event_id, device.STUDYPATIENTID, device.event_utc, device.support_type
		FROM 
			(
			SELECT event_id, STUDYPATIENTID, event_utc, result_str AS support_type FROM REMAP_CA.v3PhysioStr 
			...
			JOIN (		
				...
				FROM REMAP_CA.v3Physio WHERE sub_standard_meaning = 'Oxygen Flow Rate'
			) AS O2 ON (device.studypatientid = O2.studypatientid 
				AND TIMESTAMPDIFF(HOUR, device.event_utc, O2.event_utc) BETWEEN -12 AND 12)
		WHERE device.event_id NOT IN (SELECT event_id FROM REMAP_CA.v3OrganSupportInstance where support_type = 'HFNC')
		UNION
		SELECT 
...
DROP TABLE REMAP_CA.v3CalculatedHourlyFiO2;
	CREATE TABLE REMAP_CA.v3CalculatedHourlyFiO2 
...
# REMAP_CA.v3calculatedPFratio
DROP TABLE REMAP_CA.v3CalculatedPFratio;
	CREATE TABLE REMAP_CA.tempPnearestF
		WITH PjoinF AS (
			...
			FROM
				(SELECT * FROM REMAP_CA.v3Lab WHERE sub_standard_meaning IN ('PaO2')) AS P
				LEFT JOIN (SELECT * FROM REMAP_CA.v3CalculatedHourlyFiO2) AS F 
	...
	;
	CREATE TABLE REMAP_CA.tempPnearestE	
		WITH PjoinE AS (
			...
			FROM
				(SELECT * FROM REMAP_CA.v3Lab WHERE sub_standard_meaning IN ('PaO2')) AS P
				JOIN (SELECT * FROM REMAP_CA.v3Physio WHERE sub_standard_meaning IN ('PEEP')) AS E 
	...
	;	
	CREATE TABLE REMAP_CA.v3CalculatedPFratio
		...
		FROM REMAP_CA.tempPnearestF PF 
		LEFT JOIN REMAP_CA.tempPnearestE PE ON PF.STUDYPATIENTID = PE.STUDYPATIENTID AND PF.PaO2_utc = PE.PaO2_utc
 	;
 	DROP TABLE REMAP_CA.tempPnearestF;
	DROP TABLE REMAP_CA.tempPnearestE
; 

/* 	##################### NEEED TO ADD TO DOC below here #########################   */

### v3CalculatedPEEPjoinFiO2 ###			
DROP TABLE REMAP_CA.v3CalculatedPEEPjoinFiO2;
CREATE TABLE REMAP_CA.v3CalculatedPEEPjoinFiO2
	WITH PEEPjoinFiO2 AS (
		...
		FROM
			(SELECT * FROM REMAP_CA.v3Physio WHERE sub_standard_meaning IN ('PEEP')) AS P
			JOIN REMAP_CA.v3CalculatedHourlyFiO2 AS F 
...
### STATE HYPOXIA ###
DROP TABLE REMAP_CA.v3CalculatedStateHypoxiaAtEnroll;
	CREATE TABLE REMAP_CA.v3CalculatedStateHypoxiaAtEnroll
	WITH PFmoderate AS (
			SELECT PF.* FROM REMAP_CA.v3CalculatedPFratio PF JOIN REMAP_CA.v3RandomizedModerate R ON PF.STUDYPATIENTID = R.STUDYPATIENTID
			...
			SELECT PF.* FROM REMAP_CA.v3CalculatedPFratio PF JOIN REMAP_CA.v3RandomizedSevere R ON PF.STUDYPATIENTID = R.STUDYPATIENTID
			WHERE PF.PaO2_utc <= R.randomized_utc
		), minSevere AS (
			...
			FROM 
				(SELECT STUDYPATIENTID, event_utc FROM REMAP_CA.v3OrganSupportInstance WHERE support_type = 'IMV') AS I
				JOIN REMAP_CA.v3RandomizedSevere R ON (I.STUDYPATIENTID = R.STUDYPATIENTID)
			...
			WHERE studypatientid NOT IN (SELECT DISTINCT studypatientid FROM allJoined WHERE RandomizationType = 'Severe')
				AND studypatientid NOT IN (SELECT DISTINCT studypatientid FROM REMAP_CA.v3RandomizedModerate)
		), allJoinedPlus AS (
...
### v3CalculatedSOFA ###
DROP TABLE REMAP_CA.v3CalculatedSOFA;	
	CREATE TABLE REMAP_CA.tempKeeperMeds
		WITH med AS (
				SELECT M.StudyPatientID, M.event_utc AS med_utc, M.admin_dosage, M.units, M.route, 
					CV.display, study_day, RandomizationType
				FROM REMAP_CA.v3Med M
				JOIN CA_DATA.MAR_AD MAR ON M.event_id = MAR.event_id
				JOIN CT_DATA.CODE_VALUE CV ON (MAR.event_cd = CV.code_value) 
				JOIN REMAP_CA.v3StudyDay SD ON (M.StudyPatientId = SD.StudyPatientId 
				...
				SELECT joined.StudyPatientID, joined.study_day, minutes_since_weight, joined.converted_dose, joined.display, joined.RandomizationType
				FROM joined 
	...
	;
	CREATE TABLE REMAP_CA.v3CalculatedSOFA
		## MAP
		...
		FROM (
			...
			FROM REMAP_CA.v3Physio AS D
			JOIN REMAP_CA.v3Physio AS S ON (D.StudyPatientID = S.StudyPatientID AND D.event_utc = S.event_utc)			
			JOIN REMAP_CA.v3StudyDay SD 
		...
		FROM (
			SELECT StudyPatientID, study_day, 
				REMAP.convert_vaso_to_sofa_points(TS.sub_standard_meaning, converted_dose) AS score,  /* OLD REMAP line */ 
				RandomizationType
			FROM REMAP_CA.tempKeeperMeds AS keeper_meds
	...
	;
	DROP TABLE REMAP_CA.tempKeeperMeds;
	
### CREATE v3Hospitalization ###
DROP TABLE REMAP_CA.v3Hospitalization;
CREATE TABLE REMAP_CA.v3Hospitalization
	WITH hospitalization AS (
		...
		LEFT JOIN (
			...
			FROM REMAP_CA.v3LocOrder GROUP BY STUDYPATIENTID) AS L ON P.STUDYPATIENTID = L.STUDYPATIENTID
		LEFT JOIN (
			SELECT IM.StudyPatientID, MAX(REMAP.to_utc(CONVERT(EA.DISCH_DT_TM, DATETIME))) AS EndOfHospitalization_utc    /* OLD REMAP line */
		...
		LEFT JOIN REMAP.ManualChange_StartOfHospitalization_utc MCSH ON P.STUDYPATIENTID = MCSH.STUDYPATIENTID  /* OLD REMAP line */
	)
	SELECT 
		H.StudyPatientID, 
...
; #SELECT * FROM REMAP_CA.v3Hospitalization;


### Create v3IcuAdmitDaysOnSupport ###
DROP TABLE REMAP_CA.v3IcuAdmitDaysOnSupport; 
CREATE TABLE REMAP_CA.v3IcuAdmitDaysOnSupport
	WITH ICU_support AS (
		...
		SELECT *, LEAST(REMAP.dfltH(Vaso_min_utc), REMAP.dfltH(HFNC_min_utc), REMAP.dfltH(ECMO_min_utc), REMAP.dfltH(Niv_min_utc), REMAP.dfltH(IMV_min_utc)) AS earliest_support_utc,   /* OLD REMAP line */
			GREATEST(REMAP.dfltL(Vaso_max_utc), REMAP.dfltL(HFNC_max_utc), REMAP.dfltL(ECMO_max_utc), REMAP.dfltL(Niv_max_utc), REMAP.dfltL(IMV_max_utc)) AS lastest_suport_utc  /* OLD REMAP line */
	...
	LEFT JOIN REMAP_CA.v3RandomizedModerate M ON F.StudyPatientID = M.StudyPatientID
	LEFT JOIN REMAP_CA.v3RandomizedSevere S ON F.StudyPatientID = S.StudyPatientID
	ORDER BY StudyPatientID, stay_count
; # SELECT * FROM REMAP_CA.v3IcuAdmitDaysOnSupport;

DROP TABLE REMAP_CA.v3IcuDaysOnSupport; 
CREATE TABLE REMAP_CA.v3IcuDaysOnSupport
	WITH hrs_M AS (
		SELECT StudyPatientID, ROUND(SUM(minutes_on_support_M)/60, 0) AS hours_on_support_M 
		FROM REMAP_CA.v3IcuAdmitDaysOnSupport
		...
		SELECT StudyPatientID, (MAX(stay_count)-MIN(stay_count)+1) AS number_of_ICU_stays,
			LEAST(REMAP.dfltH(MIN(beg_utc)), REMAP.dfltH(MIN(earliest_support_utc))) AS first_ICU_admit, # either ICU min or (ED w/ organ support -> ICU admit) min    /* OLD REMAP line */
...
; # SELECT * FROM REMAP_CA.v3IcuDaysOnSupport;
